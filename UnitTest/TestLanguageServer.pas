  ...
  try
    ...
    try
      ...
      begin
        ...
        TextDocument.SignatureHelp.DynamicRegistration := False;
        ...
        TextDocument.DocumentLink.DynamicRegistration := True;
    ...
    try
      ...
      with ClientCapabilities do
      begin
        CheckEquals(True, Workspace.ApplyEdit, 'Workspace.ApplyEdit');
        CheckEquals(True, Workspace.WorkspaceEdit.DocumentChanges, 'Workspace.WorkspaceEdit.DocumentChanges');
        CheckEquals(True, Workspace.DidChangeConfiguration.DynamicRegistration, 'Workspace.DidChangeConfiguration');
        CheckEquals(True, Workspace.DidChangeWatchedFiles.DynamicRegistration, 'Workspace.DidChangeWatchedFiles');
        CheckEquals(True, Workspace.Symbol.DynamicRegistration, 'Workspace.Symbol');
        CheckEquals(True, Workspace.ExecuteCommand.DynamicRegistration, 'Workspace.ExecuteCommand');

        CheckEquals(False, TextDocument.Synchronization.DidSave, 'TextDocument.Synchronization.DidSave');
        CheckEquals(False, TextDocument.Synchronization.DynamicRegistration, 'TextDocument.Synchronization.DynamicRegistration');
        CheckEquals(True, TextDocument.Synchronization.WillSave, 'TextDocument.Synchronization.WillSave');
        CheckEquals(True, TextDocument.Synchronization.WillSaveWaitUntil, 'TextDocument.Synchronization.WillSaveWaitUntil');
        CheckEquals(False, TextDocument.Completion.DynamicRegistration);
        CheckEquals(False, TextDocument.Completion.SnippetSupport);
        CheckEquals(False, TextDocument.Hover.DynamicRegistration);
        CheckEquals(False, TextDocument.SignatureHelp.DynamicRegistration);
        CheckEquals(False, TextDocument.References.DynamicRegistration);
        CheckEquals(True, TextDocument.DocumentHighlight.DynamicRegistration);
        CheckEquals(False, TextDocument.DocumentSymbol.DynamicRegistration);
        CheckEquals(False, TextDocument.Formatting.DynamicRegistration);
        CheckEquals(True, TextDocument.RangeFormatting.DynamicRegistration);
        CheckEquals(False, TextDocument.OnTypeFormatting.DynamicRegistration);
        CheckEquals(True, TextDocument.Definition.DynamicRegistration);
        CheckEquals(False, TextDocument.CodeAction.DynamicRegistration);
        CheckEquals(False, TextDocument.CodeLens.DynamicRegistration);
        CheckEquals(True, TextDocument.DocumentLink.DynamicRegistration);
        CheckEquals(False, TextDocument.Rename.DynamicRegistration);
      end;
    finally
      ClientCapabilities.Free;
    end;
...
begin
  ...
  FLanguageServerHost.SendCompletionRequest(Uri, 7, 12);
  Response := TdwsJSONObject(TdwsJSONValue.ParseString(FLanguageServerHost.LastResponse));
  try
    CompletionListResponse := TCompletionListResponse.Create;
    try
...
begin
  BasicInitialization;
  Uri := OpenFile('ProgramAdd');
  FLanguageServerHost.SendDocumentHighlightRequest(Uri, 5, 13);
  Response := TdwsJSONObject(TdwsJSONValue.ParseString(FLanguageServerHost.LastResponse));
  try
    CheckTrue(Response['result'] is TdwsJSONArray);
    ...
    try
      ...
      CheckEquals(8, DocumentHighlight.Range.Start.Line);
      CheckEquals(13, DocumentHighlight.Range.Start.Character);
      CheckEquals(8, DocumentHighlight.Range.&End.Line);
...
var
  ...
  DocumentSymbolInformation: TDocumentSymbolInformation;
  DocumentSymbolInformationList: TDocumentSymbolInformationSortedList;
begin
  BasicInitialization;
  Uri := OpenFile('UnitAdd');
  ...
  try
    ...
    DocumentSymbolInformationList := TDocumentSymbolInformationSortedList.Create;
    try
      ...
      begin
        ...
        DocumentSymbolInformationList.Add(DocumentSymbolInformation);
      end;

      CheckEquals('A', DocumentSymbolInformationList[0].Name);
      CheckEquals(Integer(skNumber), Integer(DocumentSymbolInformationList[0].Kind));
      CheckEquals('Add', DocumentSymbolInformationList[1].Name);
      CheckEquals(Integer(skFunction), Integer(DocumentSymbolInformationList[1].Kind));
      CheckEquals('B', DocumentSymbolInformationList[2].Name);
      CheckEquals(Integer(skNumber), Integer(DocumentSymbolInformationList[2].Kind));
      CheckEquals('Result', DocumentSymbolInformationList[3].Name);
      CheckEquals(Integer(skNumber), Integer(DocumentSymbolInformationList[3].Kind));
      CheckEquals('UnitAdd', DocumentSymbolInformationList[4].Name);
      CheckEquals(Integer(skModule), Integer(DocumentSymbolInformationList[4].Kind));

      DocumentSymbolInformationList.Clean;
    finally
      DocumentSymbolInformationList.Free;
    end;
  finally
    Response.Free;
  end;
...
begin
  BasicInitialization;
  Uri := OpenFile('ProgramAdd');
  FLanguageServerHost.SendDocumentLinkRequest(Uri);
  Response := TdwsJSONObject(TdwsJSONValue.ParseString(FLanguageServerHost.LastResponse));
  try
...
begin
  ...
  try
    CheckTrue(Response['result'] is TdwsJSONArray);
    SymbolInformationArray := TdwsJSONArray(Response['result']);
    CheckEquals(2, SymbolInformationArray.ElementCount);
...
var
  UriProgram, UriUnit: string;
  Response: TdwsJSONObject;
  Location: TLocation;
begin
  BasicInitialization;
  UriProgram := OpenFile('ProgramUsesAdd');
  UriUnit := OpenFile('UnitAdd');
  FLanguageServerHost.SendDefinitionRequest(UriProgram, 3, 0);
  Response := TdwsJSONObject(TdwsJSONValue.ParseString(FLanguageServerHost.LastResponse));
  try
    Location := TLocation.Create;
    try
      ...
      CheckEquals(UriUnit, Location.Uri);
    finally
      Location.Free;
    end;
  finally
